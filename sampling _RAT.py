...
import odl
from realFanflatTransform import GeoParaSetting,FanFlatFBP,FanFlatTransform

geo_params3 = GeoParaSetting(sampleNum=500,detector_num=1024)  # 第203~826列有值
...
def init_ct_op(img,r):
  batch = img.shape[0]
  print(img.shape)
  sinogram = np.zeros([batch,500,720])
  sparse_sinogram = np.zeros([batch, 500, 720])
  ori_img = np.zeros([1, 512, 512])
  sinogram_max = np.zeros([batch, 1])
  for i in range(batch):
    print(img[i, :, 360:-360].shape)
    sinogram[i, :, :] = img[i, :, 152:-152]
    # ori_img[i,...] = Fan_FBP(sinogram[i,...]).data
    ori_img[i, ...] = FanFlatFBP(img[i, ...], geo_params3)#512 512
    sinogram_max[i, 0] = sinogram[i, ...].max()
    # sinogram[i,...] /= sinogram_max[i,0]
    t = np.copy(img[i,::r,152:-152])
    sparse_sinogram[i, ...] = resize(t, [720, 500])
...
def get_pc_sampler(sde_120,sde_720, predictor, corrector, inverse_scaler, snr,
                   n_steps=1, probability_flow=False, continuous_120=False,continuous_A=False,
  ...
  predictor_update_fn_120 = functools.partial(shared_predictor_update_fn,
                                          sde=sde_120,
                                          predictor=predictor,
                                          probability_flow=probability_flow,
                                          continuous=continuous_120)
  corrector_update_fn_120 = functools.partial(shared_corrector_update_fn,
                                          sde=sde_120,
                                          corrector=corrector,
                                          continuous=continuous_120,
                                          snr=snr,
  ...
  predictor_update_fn_720 = functools.partial(shared_predictor_update_fn,
                                          sde=sde_720,
                                          predictor=predictor,
                                          probability_flow=probability_flow,
                                          continuous=continuous_A)
  corrector_update_fn_720 = functools.partial(shared_corrector_update_fn,
                                          sde=sde_720,
                                          corrector=corrector,
                                          continuous=continuous_A,
                                          snr=snr,
  ...
  def pc_sampler(v120_model,v720_model,check_num,predict,correct):
    ...
    for p in path[:1]:
      ...
      for rid, r in enumerate(R):
        ...
        for picNO in range(1):
          with torch.no_grad():
            img = 14.6889 * loadmat("./Test_DEAD/dead_rat.mat")["sinogram"]
            img = img[None, ...]
            write_img(img[0,:,:], path=f'./batch_result1/ori{views}_and_{picNO}.png')
            ori_img, sparse_sinogram, sinogram, sinogram_max = init_ct_op(img,r)
            # write_img(sinogram[0,...],'sinogram.png')
            
            #######
            # psnr2,ssim2,mse2 = indicate(ori_img,img)
            ...
            for i in range(550,2000):
              ...
              x0[:,0,::p,:] =  x0_A[:,0,::p,:]
              #x0 = x0_A * state240
              # vec_t_90 = torch.ones(x0.shape[0], device=t_90.device) * t_90
              #vec_t_60 = torch.ones(x0.shape[0], device=t_60.device) * t_60
              vec_t_120 = torch.ones(x0.shape[0], device=t_120.device) * t_120
              #vec_t_180 = torch.ones(x0.shape[0], device=t_180.device) * t_180
              #vec_t_360 = torch.ones(x0.shape[0], device=t_360.device) * t_360

              # p2
              # x01, x0 = predictor_update_fn_60(x0, vec_t_60, model=v60_model)#1,1,768,768
              # x01, x0 = predictor_update_fn_90(x0, vec_t_90, model=v90_model)#1,1,768,768
              x01, x0 = predictor_update_fn_120(x0, vec_t_120, model=v120_model)#1,1,768,768
              #x01, x0 = predictor_update_fn_180(x0, vec_t_180, model=v180_model)#1,1,768,768
              #x01, x0 = predictor_update_fn_360(x0, vec_t_360, model=v360_model)#1,1,768,768

              ## DC
              x0[:,0,::r,:] = sinogram[:,0,::r,:]
              
              # x01, x0 = corrector_update_fn_60(x0, vec_t_60, model=v60_model)
              # x01, x0 = corrector_update_fn_90(x0, vec_t_90, model=v90_model)
              # x01, x0 = corrector_update_fn_120(x0, vec_t_120, model=v120_model)
              #x01, x0 = corrector_update_fn_180(x0, vec_t_180, model=v180_model)
              #x01, x0 = corrector_update_fn_360(x0, vec_t_360, model=v360_model)
              ...
              # #  model3  ###########################
              # x1 = np.zeros([1, 1, 768, 768], dtype=np.float32)
              # x1 = torch.from_numpy(x1).cuda()
              # x1[:,0,1::p,:] =  x0_A[:,0,1::p,:]
              # vec_t_60 = torch.ones(x1.shape[0], device=t_60.device) * t_60
              # x01, x1 = predictor_update_fn_60(x1, vec_t_60, model=v60_model)#1,1,768,768

              # ## DC
              # x1[:,0,::r,:] = sinogram[:,0,::r,:]
              
              # x01, x1 = corrector_update_fn_60(x1, vec_t_60, model=v60_model)

              # ## DC
              # state60_1 = 1-state60#1 000
              # x1 = x1 * state60_1#取第二次重建数据 1 000

              ## pading
              # x0_A = state60*x0_A#取第一次重建数据 0 111
              # x0_A[:,0,1::p,:] = x1[:,0,1::p,:]#第二次重建数据赋值给第一次7
              # ## DC
              # x0_A[:, 0,::r, :] = sinogram[:, 0,::r, :]
              ...
              if True:
                ...
                tmp = np.float32(tmp)
                img[0,:,152:-152] = tmp[0,110:-110, :]
                #print(tmp.shape)
                #tmp = np.squeeze(tmp)
                #tmp = tmp[None,...]
                for coil in range(batch_size):
                  # rec_img[coil,...] = Fan_FBP(tmp[coil,...]*sinogram_max[coil,...]).data
                  rec_img[coil,...] = FanFlatFBP(img[coil,...]*sinogram_max[coil,...],geo_params3).data
              ...
              WriteInfo(
                "./batch_result1/resultrat.csv",
                View=view,
                PSNR=psnr0, SSIM=ssim0, MSE=mse0,
                File=picNO, Check_num_hh=check_num[0], Check_num_3l=check_num[1], Predictor=set_predict(predict),
                Corrector=set_correct(correct))

            print("MAX:  PSNR:{} SSIM:{} MSE:{}".format(np.round(max_psnr[:4],3),np.round(max_ssim[:5],4),np.round(1000*min_mse[:4],3)))
            # assert False
            np.save('./batch_result1/picNO_{}_check_{}_view_{}_batch_img_2000.npy'.format(picNO,check_num,view),best_img)
            print(best_img.shape)
            write_img(best_img[0,...], path=f'./batch_result1/sparse_sinogram_{view}_and_{picNO}.png')
            
            #for file_id,name in enumerate(file_name):
            #for file_id in picNO:
            WriteInfo(
              "./batch_result1/resultrat1.csv",
              View=view,
