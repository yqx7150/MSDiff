...
from sampling import init_ct_op
...
class GetCT(Dataset):
    ...
    def padding_img(self,img):
        w,h = img.shape
        h1 = (h//64 + 1)*64
        tmp = np.zeros([h1,h1], dtype=np.complex64)
        x_start = int((h1 -w)//2)
        y_start = int((h1 -h)//2)
    ...
    def __getitem__(self,index):
        ...
        r = 6
        sparse_sinogram = np.zeros([1, 720, 720])
        # img = loadmat(self.data_names[index])["data"]
        img = np.load(self.data_names[index])
        sinogram = Fan_ray_trafo(img).data
        #write_img(sinogram, 'rec' + str(index) + '_img.png')
        print(sinogram.shape)  #720 720
        sinogram = sinogram[ None, ...]
        # sinogram = sinogram / sinogram.max()
        #########################aug##########################
        #sinogram = np.squeeze(sinogram)
        # sparse_sinogram[0,::r, :] = sinogram[0,::r, :]
        sinogram[0,::r, :] = sparse_sinogram[0,::r, :]
        print(sinogram.shape) # 1 720 720

        sinogram = np.squeeze(sinogram)
        ########################################################
        sinogram = self.padding_img(sinogram)
        #ori_data = torch.tensor(sinogram).cuda()
        #write_img(sparse_sinogram, 'ori' + str(index) + '_img.png')

        ori_data = sinogram[None, ...]
        print(ori_data.shape)  #1 768 768
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  test_dataset = GetCT(root= "./test_img")
